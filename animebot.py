...
def set_bot_commands(token):
    ...
    commands = [
        ...
        {'command': 'removereminder', 'description': 'Remove a reminder for a specific anime'},
        {'command': 'help', 'description': 'Show help to user'},
        {'command': 'owner', 'description': 'Show owners info'}
    ]
...
def main():
    ...
    def check_reminders_job():
        asyncio.run(scheduled_check_reminders())
    
    scheduler.add_job(check_reminders_job, IntervalTrigger(minutes=1))
    scheduler.start()
